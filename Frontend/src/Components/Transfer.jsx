...
export function Transfer() {
    ...
    const user = searchPararms.get("user")
    const token = `Bearer ${localStorage.getItem("token")}`
    const to = searchPararms.get("to")

    async function sendMoney() {
        try {
            const response = await axios.post(`${LINK}/transaction?username=${user}&to=${to}&amount=${amount}`, { headers: { token: token } })
                ...
                <div className="p-6">
                    <div className="flex items-center space-x-4">
                        <div className="w-12 h-12 rounded-full bg-green-500 flex items-center justify-center">
                            <span className="text-2xl text-white">{to[0].toUpperCase()}</span>
                        </div>
